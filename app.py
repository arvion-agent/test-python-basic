...
class ItemsView(views.MethodView):
    def get(self):
        # Using connection-based execution instead of ORM execute
...
def bulk_create_items():
    """Bulk create items using raw SQL with connection-based execution"""
...
def seed_database():
    ...
    if count == 0:
        # Insert seed data using connection-based execution within a transaction
        with engine.begin() as conn:
            conn.execute(
                text('INSERT INTO item (id, name) VALUES (:id, :name)'),
                [

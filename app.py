...
def get_all_items_raw():
    """Get all items using raw SQL with connection.execute"""
...
def get_item_by_id_raw(item_id):
    """Get a specific item using raw SQL with connection.execute"""
    with engine.connect() as conn:
...
def create_item_raw(item_data):
    """Create a new item using raw SQL with connection.execute"""
...
class ItemsView(views.MethodView):
    def get(self):
        # Using connection.execute instead of ORM
        items = get_all_items_raw()
        return jsonify(items)
    
    def post(self):
        new_item = json.loads(request.data)
        # Using connection.execute for insert
...
def get_item(item_id):
    # Using connection.execute to fetch the item
...
def bulk_create_items():
    """Bulk create items using connection.execute"""
...
def seed_database():
    # Check if we already have items using connection.execute
    with engine.connect() as conn:
        result = conn.execute(text('SELECT COUNT(*) FROM item'))
        count = result.scalar()
    
    if count == 0:
        # Insert seed data using connection.execute
        with engine.begin() as conn:
            conn.execute(
                text('INSERT INTO item (id, name) VALUES (:id, :name)'),
                [

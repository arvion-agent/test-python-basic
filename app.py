...
class ItemsView(views.MethodView):
    def get(self):
        # Using connection-based execution instead of ORM direct engine.execute
...
def bulk_create_items():
    ...
    # Use executemany for bulk operations using a transaction
    with engine.begin() as conn:
        conn.execute(
            text('INSERT INTO item (id, name) VALUES (:id, :name)'),
            items_to_insert
...
# Seed the database with initial data
 def seed_database():
    ...
    if count == 0:
        # Insert seed data using a transaction
        with engine.begin() as conn:
            conn.execute(
                text('INSERT INTO item (id, name) VALUES (:id, :name)'),
                [
